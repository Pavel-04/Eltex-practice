...
int main(){
    ...
    if ((mq_send_fd = mq_open(QUEUE_A_TO_B,O_CREAT| O_WRONLY, 0600,&attr)) == (mqd_t)-1){
        perror("Creating queue error");
        return -1;
    }
    if ((mq_receive_fd = mq_open(QUEUE_B_TO_A,O_CREAT | O_RDONLY, 0600,&attr)) == (mqd_t)-1){
        perror("Creating queue error");
        return -1;
    }
