...
int isPIDUniqueR(Node* node, const char* pid) {
    ...
    if (strcmp(node->contact.PID, pid) == 0) {
        return 0;
    }
    
    // Если нашли в левом поддереве, не проверяем правое
    if (!isPIDUniqueR(node->left, pid)) {
        return 0;
    }
    
    // Проверяем правое поддерево
    return isPIDUniqueR(node->right, pid);
}

int isPIDUnique(Btree* tree, const char* pid) {
    if (tree == NULL || tree->root == NULL || pid == NULL) {
        return 1;
    }
    
    return isPIDUniqueR(tree->root, pid);
}
...
int addContact(Btree* btree) {
    ...
    do {
        ...
        if (!isPIDUnique(btree, newContact.PID)) {
            printf("Ошибка: PID '%s' уже существует! Введите другой PID.\n", newContact.PID);
        }
    } while (!isPIDUnique(btree, newContact.PID));
    ...
    int result = addNode(btree, newContact);
...
Node* findContactR(Node* node, const char* PID){
    ...
    Node* leftResult=findContactR(node->left,PID);
    if(leftResult != NULL)
    {
        return leftResult;
    }
    return findContactR(node->right,PID);
}
Node* findContact(Btree* btree, const char* PID){
    if (btree == NULL || btree->root == NULL || PID == NULL) {
        return NULL;
    }
    
    return findContactR(btree->root, PID);
}
...
int deleteContact(Btree* btree) {
    ...
    PID[strcspn(PID, "\n")] = 0;
    Node* current = findContact(btree, PID);
    if (current == NULL) {
        return CONTACT_NOT_FOUND;
    }
    return deleteNode(btree, current);
}
...
int editContact(Btree* btree) {
    ...
    int result = editNode(btree, current, newContact);
    freeContact(&newContact);
